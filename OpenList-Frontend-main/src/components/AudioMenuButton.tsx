...
export function AudioMenuButton(props: AudioMenuButtonProps) {
  const t = useT()
  ...
  const handleAddToFavorites = async () => {
    if (!selectedFolderId()) {
      notify.error(t("home.audio_favorites.select_folder_first"))
      return
    }
    ...
    try {
      ...
      notify.success(t("home.audio_favorites.audio_added"))
      setNote("")
      addModal.onClose()
      ...
      if (error?.message?.includes("already exists")) {
        notify.error("该音频已在此文件夹中")
      } else {
        notify.error(t("home.audio_favorites.failed_add_audio"))
      }
    }
  }
  ...
  return (
    <>
      <Box class="audio-menu-button">
        <Button
          size="sm"
          variant="outline"
          onClick={addModal.onOpen}
        >
          {t("home.audio_menu.more")}
        </Button>
      </Box>
          ...
          <ModalBody>
            <VStack spacing="$4" alignItems="stretch">
              <Button
                ...
                w="$full"
              >
                {t("home.audio_menu.view_favorites")}
              </Button>
                ...
                <Show
                  ...
                  fallback={
                    <VStack spacing="$3" alignItems="stretch">
                      <input
                        type="text"
                        placeholder={t("home.audio_favorites.folder_name")}
                        value={newFolderName()}
                      ...
                      />
                      <Textarea
                        placeholder={t("home.audio_favorites.folder_description")}
                        value={newFolderDesc()}
                ...
                >
                  <VStack spacing="$3" alignItems="stretch">
                    <Show
                      ...
                      fallback={
                        <Text color="$neutral11">
                          {t("home.audio_favorites.no_folders")}
                        </Text>
                      }
                    >
                      <Box>
                        <Text fontSize="$sm" fontWeight="$medium" mb="$2">
                          {t("home.audio_favorites.select_folder")}
                        </Text>
                        <select
                      ...
                      <Box>
                        <Text fontSize="$sm" fontWeight="$medium" mb="$2">
                          {t("home.audio_favorites.note_optional")}
                        </Text>
                        <Textarea
                          placeholder={t("home.audio_favorites.add_note")}
                          value={note()}
                    ...
                    <Button
                      ...
                      size="sm"
                    >
                      {t("home.audio_favorites.create_new_folder")}
                    </Button>
                  </VStack>
                </Show>
              </Box>
